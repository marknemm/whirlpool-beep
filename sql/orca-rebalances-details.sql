SELECT
  ...
  ,liquidity_deposit.token_amount_a / (10 ^ token_a.decimals)       AS sol_deposit
  ,liquidity_deposit.token_amount_b / (10 ^ token_b.decimals)       AS usdc_deposit
  ,liquidity_deposit.usd                                            AS usd_deposit
  ,liquidity_withdraw.token_amount_a / (10 ^ token_a.decimals) * -1 AS sol_withdraw
  ,liquidity_withdraw.token_amount_b / (10 ^ token_b.decimals) * -1 AS usdc_withdraw
  ,liquidity_withdraw.usd * -1                                      AS usd_withdraw
  ,fee_reward.token_amount_a / (10 ^ token_a.decimals)              AS sol_collected
  ,fee_reward.token_amount_b / (10 ^ token_b.decimals)              AS usdc_collected
...
WHERE position.id NOT IN (
  ...
  HAVING ABS(SUM(liquidity.usd)) > 5
)
